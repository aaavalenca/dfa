        ...
        ZStack{
            ...
            VStack{
                        ...
                        GeometryReader {geo in
                            // Level 1 only has one picker
                                ...
                                MenuPicker(bodies: $bodies, selectedBody: $selectedBody5)
                                    .frame(width: geo.size.width * 0.2, height: geo.size.width * 0.2)
                                    .position(x: geo.size.width * viewModel.allLevels[level].pickerXPos[4], y: geo.size.height * viewModel.allLevels[level].pickerYPos[4])
                                MenuPicker(bodies: $bodies, selectedBody: $selectedBody6)
                                    .frame(width: geo.size.width * 0.2, height: geo.size.width * 0.2)
                                    .position(x: geo.size.width * viewModel.allLevels[level].pickerXPos[5], y: geo.size.height * viewModel.allLevels[level].pickerYPos[5])
                ...
                HStack{
                    ...
                    NavigationLink(destination: GoodbyeView(isActive: $isActive), isActive: $next) { EmptyView()}
